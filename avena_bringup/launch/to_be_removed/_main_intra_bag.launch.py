...
def generate_launch_description():
    ...
    data_streams = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(os.path.join(get_package_share_directory(
            'avena_bringup'), 'launch', 'data_streams_bag.launch.py'))
    )
    robot_state_publisher = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(os.path.join(get_package_share_directory(
            'avena_bringup'), 'launch', 'franka.launch.py'))
    )
    ...
    return LaunchDescription([parameters_server, data_streams, robot_state_publisher,
                              generate_objects_pipeline,
                              get_occupancy_grid, item_select, gripper_controller, arm_controller, cli_manager, path_buffer])
