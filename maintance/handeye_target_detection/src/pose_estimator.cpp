...
PoseEstimator::PoseEstimator(std::shared_ptr<rclcpp::Node> &node, std::string pattern, std::string image_topic,
                             ...
                             std::string marker_frame_name)
    : node_(node), it_(node_), broadcaster_(node_), run_(false), width_(width), height_(height),
      chessboard_square_size_(chessboard_square_size), circle_grid_seperation_(circle_grid_seperation),
      aruco_board_marker_size_(aruco_board_marker_size), aruco_board_marker_seperation_(aruco_board_marker_seperation),
      charuco_board_marker_size_(charuco_board_marker_size), charuco_board_square_size_(charuco_board_square_size),
      marker_frame_name_(marker_frame_name)
{
    ...
    camerainfo_sub_ = node_->create_subscription<sensor_msgs::msg::CameraInfo>(
        camera_info_topic, 1, std::bind(&PoseEstimator::caminfoCB, this, std::placeholders::_1));
    ...
    const rclcpp::QoS qos_settings = rclcpp::QoS(rclcpp::KeepLast(10)).best_effort().durability_volatile();
    switch (calibration_pattern_)
    {
    case CHESSBOARD:
        image_sub_ = node_->create_subscription<sensor_msgs::msg::Image>(image_topic, qos_settings, std::bind(&PoseEstimator::imageCB_CHESSBOARD, this, std::placeholders::_1));
        // image_sub_ = it_.subscribe<PoseEstimator>(image_topic, 1, &PoseEstimator::imageCB_CHESSBOARD, this);
        break;
    case ASYMMETRIC_CIRCLES_GRID:
        RCLCPP_ERROR(node_->get_logger(), "Callback not supported. Change calibration pattern. Exiting...");
        rclcpp::shutdown();
        std::exit(1);
...
void PoseEstimator::imageCB_CHESSBOARD(const sensor_msgs::msg::Image::SharedPtr msg)
{
    if (run_)
    {
        if (!msg)
            ...
            if (found)
            {
                ...
                if (solved)
                {
                    ...
                    tf2::transformTF2ToMsg(tf2::Transform(q, tf2::Vector3(tvect[0], tvect[1], tvect[2])), transform_stamped,
                                           node_->now(), msg->header.frame_id, marker_frame_name_);
                ...
                // Output stream
                cv_image_color.header.frame_id = "world";
                image_pub_.publish(cv_image_color.toImageMsg());
            }
            else
            {
                RCLCPP_ERROR(node_->get_logger(), "Cannot find marker");
                // Publish marker not found image
                cv::Mat img = cv::Mat::zeros(720, 1280, CV_8UC1);
                const std::string text = "Marker not found!";
                ...
                cv::Point text_org((img.cols - text_size.width) / 2,
                                   (img.rows + text_size.height) / 2);
                cv::putText(img,      //target image
                            text,     //text
                            text_org, //top-left position
                            font_face,
                            font_scale,
                            cv::Scalar(255),
                            thickness);

                sensor_msgs::msg::Image out_ros_image;
                helpers::converters::cvMatToRos(img, out_ros_image);
                out_ros_image.header.stamp = node_->now();
                out_ros_image.header.frame_id = "world";
                image_pub_.publish(out_ros_image);
            }
        }
