...
int Diagnostics::writeDiagnostics()
{
    for (size_t jnt_idx = 0; jnt_idx < _joints_number; jnt_idx++)
    {
        ...
        namestring << _serial_number << std::put_time(now, "_%y_%m_%d_%H%M") << '_' << std::to_string(jnt_idx);
        std::string filename;
        namestring >> filename;

        std::cout << std::string("/home/ubuntu/joint_diagnostic/diagnostic_data/") + _serial_number + std::string("/") + filename + std::string(".txt") << std::endl;
        std::ofstream chart(std::string("/home/ubuntu/joint_diagnostic/diagnostic_data/") + _serial_number + std::string("/") + filename + std::string(".txt"));
        chart.precision(2);
        chart << std::string("serial number: ") << _serial_number << std::endl;
        chart <<std::setprecision(5)<< std::string("torque: ") << _const_torque << std::string("\nstart_temp: ") << _start_temp[jnt_idx] << " (" << _start_motor_temp[jnt_idx] <<")"
        << std::string("\nfinish_temp: ") << _arm_status.joints[jnt_idx].temperature <<" ("<< _arm_status.joints[jnt_idx].motor_temperature<<")"<<std::endl;
        chart << std::string("note: ") << _note << std::endl;
        chart.precision(6);
...
void Diagnostics::controlLoop()
{
    ...
    // }
    getAverageArmState();
    if ((_t_current - _t_start) > std::chrono::seconds(5))
        saveDiagnostics();
...
int Diagnostics::jointInit()
{
    ...
    for (size_t jnt_idx = 0; jnt_idx < _joints_number; jnt_idx++)
    {
        _start_motor_temp.push_back(_arm_status.joints[jnt_idx].motor_temperature);
...
int Diagnostics::paramInit()
{
    ...
    _node->declare_parameter<std::string>("serial_number", "NA");
    _node->declare_parameter<std::string>("note", "NA");
    ...
    _node->get_parameter("serial_number", _serial_number);
    _node->get_parameter("note", _note);
