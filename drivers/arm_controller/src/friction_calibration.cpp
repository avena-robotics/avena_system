...
int FrictionCalibration::jointInit()
{
    ...
    if (_joints_number-1){
        RCLCPP_ERROR(_node->get_logger(),"Please connect only one joint");
        std::exit(1);
    }
...
void FrictionCalibration::init()
{
    ...
    for (int asdf = 0; asdf < 12; asdf++)
    {
        ...
        double vel_increment = 0.05 * direction, max_vel = 0.5 * direction;
        ...
        for (size_t jnt_idx = 0; jnt_idx < _joints_number; jnt_idx++)
        {
            ...
            set_vel[jnt_idx] = 0.5 * direction * -1;
        ...
        for (size_t jnt_idx = 0; jnt_idx < _joints_number; jnt_idx++)
        {
            ...
            int file_i = 0;
            while (std::filesystem::exists(std::string("/home/ubuntu/joint_diagnostic/friction_data/") + _serial_number + std::string("/") + std::to_string(file_i) + std::string(".txt")))
                file_i++;
            std::cout << std::string("/home/ubuntu/joint_diagnostic/friction_data/") + _serial_number + std::string("/") + std::to_string(file_i) + std::string(".txt") << std::endl;
            chart = std::make_shared<std::ofstream>(std::string("/home/ubuntu/joint_diagnostic/friction_data/") + _serial_number + std::string("/") + std::to_string(file_i) + std::string(".txt"));
            chart->precision(6);

            std::cout << "writing " <<  _measured_friction_comp[jnt_idx].size() << " out of 22 measurements" << std::endl;
            std::cout << asdf+1 << " out of 12 cycles done" << std::endl;
    ...
    done_zero=true;
    for (size_t jnt_idx = 0; jnt_idx < _joints_number; jnt_idx++){
	if(std::abs(_arm_status.joints[jnt_idx].position)>0.1){
	    _arm_command.joints[jnt_idx].c_torque=0.1;
	    done_zero=false;
	}else
	    {
	    _arm_command.joints[jnt_idx].c_torque=0.;
