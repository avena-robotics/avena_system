...
struct JointStatus
{
    double position, velocity, torque, temperature, motor_temperature, ma_val;
...
private:
    ...
    void commsLoop()
    {
        ...
        if (rclcpp::ok())
        {
            ...
            if (!sendArmConfig(_arm_config, 2*_can_loop_t))
            {
                if (!sendArmState(_arm_state_command, 2*_can_loop_t))
                {
    ...
    bool updateArmState()
    {
...

        {
            ...
            _arm_status->joints[arm_id].motor_temperature = _last_msg.rx_msgs[i][18];
